  ...
  <div class="DB-information">
    ...
    <div class="data-table-container">
      <div class="table-header">
        <div class="header-item" style="width: 15%">成就名称</div>
        <div class="header-item" style="width: 20%">获取方式</div>
        <div class="header-item" style="width: 25%">成就描述</div>
        <div class="header-item sortable" style="width: 10%">
          玩家进度
          <div class="sort-icon" @click="toggleSort">
            <UpDownArrow />
          </div>
        </div>
        <div class="header-item sortable" style="width: 20%">关键词</div>
      ...
      <div class="table-body">
        ...
        >
          <div class="data-cell" style="width: 15%">
            <div class="scrollable-text">{{ item.name }}</div>
          </div>
          <div class="data-cell" style="width: 20%">
            <div class="scrollable-text">{{ item.access }}</div>
          </div>
          <div class="data-cell" style="width: 25%">
            <span class="scrollable-text">{{ item.description }}</span>
          </div>
          <div class="data-cell" style="width: 10%">
            <span class="scrollable-text">{{ item.complete_percentage }} %</span>
          </div>
          <div class="data-cell" style="width: 20%">
...
import {type AchievementsDBInfo, deleteAchievementsData} from "../../apis/database.ts";
import {updateAchievementsData} from '../../apis/database.ts';
import EditAchievementsDataForm from "../Form/EditAchievementsDataForm.vue";

const pageInput = ref(1); // 页码输入框的值
const sortOrder = ref<'asc' | 'desc' | null>(null); // 简化排序状态
const editingDB = ref<AchievementsDBInfo>(); // 用于存储正在编辑修改的数据
...
const paginatedData = computed(() => {
  ...
  if (sortOrder.value === 'asc') {
    sortedItems.sort((a, b) => a.complete_percentage - b.complete_percentage);
  } else if (sortOrder.value === 'desc') {
    sortedItems.sort((a, b) => b.complete_percentage - a.complete_percentage);
  }
...
// 操作函数
const editItem = (item: AchievementsDBInfo) => {
  editingDB.value = item;
  isEditing.value = true; // 打开编辑弹窗
};

// 修改数据库内容
const handleUpdate = async (data: AchievementsDBInfo) => {
  try {
    isWaiting.value = true;
    await updateAchievementsData(data);
...
// 删除函数
const deleteItem = async (item: AchievementsDBInfo) => {
