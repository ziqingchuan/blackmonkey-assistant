...
const props = defineProps({
  visible: Boolean,
  editingData: {
    type: Object as () => CombatKnowledgeDBInfo | null,
    default: null
  }
});
...
const formData = ref<CombatKnowledgeDBInfo>({
  primary_key: '',
  title: '',
  content: '',
  category: '',
...
watch(() => props.editingData, (newVal) => {
  if (newVal) {
    ...
    keywordsString.value = newVal.keywords.join(', ');
    prerequisitesString.value = newVal.prerequisites.join(', ');
    relatedSkillsString.value = newVal.related_skills.join(', ');
  }
});
