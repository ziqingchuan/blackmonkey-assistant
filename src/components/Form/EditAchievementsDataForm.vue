  ...
  <div class="edit-dialog-overlay" v-if="visible">
    <div class="edit-dialog">
      ...
      <div class="dialog-body">
        <div class="form-item">
          <label>成就名称</label>
          <textarea v-model="formData.name" class="ancient-input"></textarea>
        </div>
        <div class="form-item">
          <label>获取方式</label>
          <input type="text" v-model="formData.access" class="ancient-input">
        </div>
        <div class="form-item">
          <label>成就描述</label>
          <input type="text" v-model="formData.description" class="ancient-input">
        </div>
        <div class="form-item">
          <label>玩家进度</label>
          <input type="text" v-model="formData.complete_percentage" class="ancient-input">
        </div>
        <div class="form-item">
          <label>关键词</label>
          <input type="text" v-model="formData.keywords" class="ancient-input">
        </div>
      </div>
...
import type { AchievementsDBInfo } from '../../apis/database.ts';
import CustomAlert from "../Dialog/CustomAlert.vue";
const customAlert = ref(); // 获取弹窗组件的引用
const props = defineProps({
  visible: Boolean,
  editingData: {
    type: Object as () => AchievementsDBInfo | null,
    default: null
  }
});
...
const handleConfirm = () => {
  if (!formData.value.name || !formData.value.description || !formData.value.complete_percentage || !formData.value.access || !formData.value.keywords) {
